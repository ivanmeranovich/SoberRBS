...
def main():
    parser = argparse.ArgumentParser(
        description="Скрипт ищет окно по заданной нуклеотидной последовательности (мотив) в GenBank-файле.\n"
                    ...
                    "Флаг --help выводит расширенную справку о работе скрипта.\n\n"
                    "Пример вызова:\n"
                    "  python script.py input.gb AGGAGG output.gb [--test] [--help]",
        add_help=False
    )
    # Добавляем наш собственный флаг для расширенной справки.
    ...
    # Если флаг --help указан, выводим расширенную справку и завершаем работу.
    if args.help:
        print("Расширенная справка:\n")
        print("Этот скрипт выполняет поиск окна по заданной нуклеотидной последовательности (мотиву) во входном GenBank-файле.\n"
              "Для каждого окна длины искомой последовательности вычисляется процент сходства с мотивом.\n"
              "В обычном режиме аннотируются только те окна (выводимые как фичи типа RBS), которые удовлетворяют условиям:\n"
              "  – сходство с искомой последовательностью не менее 80%;\n"
              "  – окно не находится внутри CDS;\n"
              "  – минимальное расстояние от окна до ближайшей границы CDS не превышает 40 нуклеотидов.\n\n"
              "При использовании флага --test условия по CDS и расстоянию игнорируются, и аннотируются все окна с нужным сходством.\n\n"
              "Пример вызова:\n"
    ...
    proximity_threshold = 40  # порог установлен равным 40 нуклеотидам
    ...
    # Получаем интервалы для всех CDS
    cds_intervals = []
    ...
    annotated_count = 0
    if args.test:
        logging.info("Тестовый режим: аннотируются все вхождения (с сходством ≥ 80%), условия по CDS и дистанции игнорируются.")
